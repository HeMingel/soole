...
//                        retStr = wxPayNotifyProcess("FAIL", "通知支付金额与订单金额不符");
    ...
    public String wxPayNotifyNew(HttpServletRequest request) {
        ...
        log.info("微信支付通知retStr is:{}", retStr);
        try (InputStream is = request.getInputStream()) {
            log.debug("微信支付通知1111111111111111");
            // 读取支付回调参数
            byte[] bytes = IOUtils.readFully(is, request.getContentLength());
            log.debug("微信支付通知2222222222222222222");
            if (bytes.length > 0) {
                log.debug("微信支付通知33333333333333");
                String result = new String(bytes, StandardCharsets.UTF_8);
                log.debug("微信支付通知44444444444444444444444444");
                if (StringUtils.isNotBlank(result)) {
                    log.debug("微信支付通知5555555555555555555555555555");
                    // 支付回调参数
                    Map<String, String> resParams = WXPayUtil.xmlToMap(result);
                    log.debug("微信支付通知6");
                    log.debug("微信支付通知回调参数: {}", resParams);
